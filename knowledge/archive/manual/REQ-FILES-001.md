...
requirement: ../../01-product/requirements/files.md
sources:
...
This proposal translates approved requirement `REQ-FILES-001` into a manual
verification procedure. Confirm the role-operation matrix and exact UI/API
error wording during human review before promotion.
...
- The API file spec demonstrates safe storage-name handling, viewer upload
  rejection, missing-folder rejection, and successful listing.
- The UI file spec demonstrates successful editor and admin uploads and the
  visible success/file-name outcomes.
- The current automated evidence does not independently establish preview,
  download, rename, delete, or the complete role-operation matrix. Reviewers
  should confirm the available product surface and exact expected errors before
