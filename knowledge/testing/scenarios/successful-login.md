...
freshness: fresh
product_features:
  - ../../product/features/login.md
expected_behavior:
  - ../../product/expected-behavior/login-success.md
sources:
...
- Authentication route used by the application: `POST /auth/login`

## Verification

`LoginPage.login(...)` fills the username and password, submits the form, waits for `HomePage`, and returns that Page Object. The test then asserts that `HomePage.title` is visible. This verifies the successful-login transition and the visible workspace-home outcome for the configured admin account.

Related product knowledge:

- [Login feature](../../product/features/login.md)
- [Successful login behavior](../../product/expected-behavior/login-success.md)
